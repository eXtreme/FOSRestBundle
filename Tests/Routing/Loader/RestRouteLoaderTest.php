...
class RestRouteLoaderTest extends LoaderTest
{
     ...
     * parameters.
     */
    public function testPsr7RequestTypeHintsIgnoredCorrectly()
    {
        if (!interface_exists('Psr\Http\Message\MessageInterface')) {
            $this->markTestSkipped('A "psr/http-message" package must be installed.');
        }

        $collection = $this->loadFromControllerFixture('TypeHintedPsr7Controller');
